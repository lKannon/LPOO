			...
			if (linha != null) {
				...
				if (k > 0) {
					...
					if (parcelas.length > 1) {// encontra algo
						...
						new CompraCartaoCreditoParcelada(array[0], array[2], -Float.parseFloat(array[3]), conta,
								nVezes);
					} else {
						nVezes = 1;
						new CompraCartaoCreditoAVista(array[0], array[2], -Float.parseFloat(array[3]), conta);
					}
		...
		while (true) {
			...
			if (linha != null) {
				...
				if (k > 0) {
					...
					new CompraCartaoDebito(array[0], array[3], Float.parseFloat(array[1]), conta);
				}
